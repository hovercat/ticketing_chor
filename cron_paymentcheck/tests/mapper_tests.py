...
def connect_to_db(SQL_CONNECTOR):
    engine = db.create_engine(SQL_CONNECTOR, connect_args={'options': '-csearch_path=ticketing'})  # schema name
...
def create_db(DB, USR, HOST, PORT, DO_INSERTS=False):
    conn = connect(
        dbname=DB,
        user=USR,
        # password = PW,
...
class CheckerTests(unittest.TestCase):
    ...
    def test_check_reservation_finalize(self):
        ...
        checker.check_reservation(self.db, self.res)
    ...
    def test_check_reservation_remind(self):
        self.res.status = 'open'
    ...
    def test_check_reservation_cancel(self):
